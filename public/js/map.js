(function () {
  ...
  L.control.layers({ 'Street': streetLayer, 'Satellite': satelliteLayer }, {}, { position: 'topright', collapsed: false }).addTo(map);
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        hint.textContent = 'Tap anywhere on the map to log a card there.';
      },
      () => { hint.textContent = "Couldn't find your location — scroll the map, then tap a spot to log a card."; },
  ...
  document.addEventListener('submit', function (e) {
    ...
    delete payload.files; // file input isn't part of the JSON create call

    // Clean up mutually-exclusive fields based on what's actually selected
